	...
	public override void OnXGUI(){
		...
		if( null == dbFile ){
			...
			if(!AssetDatabase.GetAssetPath(dbFile).Contains(".db")){
				...
				isDirty = false;
				string DB_Name = "TinyTimes.db";
				string appDBPath = Application.dataPath + "/StreamingAssets/" + DB_Name;
		        CurrentDataBase = new DbAccess("URI=file:" + appDBPath);
		        allTable.Clear();
		        currTable = "";
				// CurrentDataBase = new DbAccess(AssetDatabase.GetAssetPath(dbFile));
			}
			...
			if( null != CurrentDataBase ){
				if( CreateSpaceButton("DataBase") ){
					...
					SqliteDataReader recordTableReader = CurrentDataBase.ExecuteQuery (GET_ALL_TABLE);
				...
				for(int pos = 0; pos < allTable.Count; pos++){
					if(CreateSpaceButton(allTable[pos])){
						...
						SqliteDataReader recordTableReader = CurrentDataBase.ExecuteQuery (sql);
